    ...
    <div class="flex min-h-screen">
        ...
        <aside class="w-64 bg-sidebar text-slate-200 flex flex-col fixed inset-y-0 left-0">
            <div class="flex items-center gap-3 px-5 py-5 border-b border-white/10">
                ...
                @else
                    <div class="w-9 h-9 rounded-lg bg-brand flex items-center justify-center font-bold text-white">EF
                    </div>
                @endif
                <div>
            ...
            <div class="px-3 py-4 border-t border-white/10 space-y-1">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button
                        class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-slate-300 hover:bg-white/5 transition">
        ...
        <div class="flex-1 ml-64 flex flex-col min-h-screen">
            ...
            <header
                ...
                <div class="relative w-96 max-w-full">
                    ...
                    <input type="text" placeholder="Search students, faculty, or resources..."
                        class="w-full pl-9 pr-3 py-2 rounded-lg bg-slate-100 text-sm border-none focus:ring-2 focus:ring-brand focus:bg-white transition">
                </div>
                <div class="flex items-center gap-4">
                    ...
                    <a href="{{ route($role . '.profile.edit') }}" class="flex items-center gap-2">
                        @if (auth()->user()->photo)
                            <img src="{{ asset('storage/' . auth()->user()->photo) }}" alt="Profile photo"
                                class="w-9 h-9 rounded-full object-cover">
                        @else
                            <div
                                class="w-9 h-9 rounded-full bg-brand text-white flex items-center justify-center font-semibold text-sm">
