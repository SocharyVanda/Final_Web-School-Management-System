...
@section('content')
<div class="max-w-3xl">
    <h1 class="text-2xl font-bold text-slate-800 mb-1">Edit Student</h1>
    <p class="text-sm text-slate-500 mb-6">Update {{ $student->user->name }}'s record.</p>

    <form method="POST" action="{{ route('admin.students.update', $student) }}" enctype="multipart/form-data" class="bg-white rounded-card shadow-soft p-6 space-y-5">
        @csrf @method('PUT')
    ...
    <div class="bg-white rounded-card shadow-soft p-6 mt-6">
        ...
        <form method="POST" action="{{ route('admin.students.reset-password', $student) }}" class="flex gap-3">
            @csrf
